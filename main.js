    ...
    for (let i = 1; i <= 3; i++) {
      ...
      if (container) {
        ...
        img.onerror = function () {
          ...
          this.src =
            "https://via.placeholder.com/400x300/1a1a2e/e0e0ff?text=Cyber+Image";
        };
 ...
 (    )----(    )
  ...
  const sequences = [
    ...
    () => {
      ...
      addTerminalElement(
        "quote",
        headroomQuotes[Math.floor(Math.random() * headroomQuotes.length)],
        1.2
      );
      addTerminalElement(
        "ascii",
    ...
    () => {
      addTerminalElement(
        "prompt",
        "$ 🌐🗑~ Questioning patterns...or questioning you?",
        0
      );
      addTerminalElement(
        "thought",
  ...
  cardButtons.forEach((button) => {
    button.addEventListener("click", (e) => {
      ...
      const randomMessage =
        productMessages[cardId][
          Math.floor(Math.random() * productMessages[cardId].length)
        ];
      const cardNumber = card.querySelector(".random-id").textContent;

      addTerminalElement(
        "system",
        `$ 🌐🗑~ Examining ${cardTitle.toUpperCase()} (#${cardNumber})...`,
        0
      );
